...
export class TokenService {
  ...
  public tokenPostCall(callback){
    let globalData:any = globalVars;
    ...
    if(!globalData.refresh_token){
      ...
      params = new HttpParams({
        fromObject: {
          ...
          refresh_token: globalData.refresh_token,
        }
      });
    }
    
    let takenConfig = new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded');
    this.http.post(getTokenURL,params,{ headers: takenConfig }).subscribe((res:any)=>{  
      if(res){
  ...
  modalOpenMethod(message:any){
    let userData:any = JSON.parse(localStorage.getItem("userObj"));
    userData.modalObj = {};
    userData.modalObj.content = message;
